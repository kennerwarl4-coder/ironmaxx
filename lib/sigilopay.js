...
function buildPixPayload({ amount, shippingFee, client, items, utm }) {
  ...
  return {
    ...
    // Não mandamos callbackUrl aqui: a SigiloPay registra um webhook novo a cada
    // chamada que inclui esse campo, e a conta tem um teto de 20 webhooks —
    // configure a URL de callback uma única vez no painel da SigiloPay
    // (https://SEU_DOMINIO/api/webhooks/sigilopay) em vez de por requisição.
  };
}
